...
export async function POST(req: Request) {
  ...
  const kind = new URL(req.url).searchParams.get("kind") ?? "question";
  const safeName = file.name.replace(/[^a-zA-Z0-9_.-]/g, "_");
  const prefix = questionId ? `questions/${questionId}` : "questions/misc";
  const path = `${prefix}/${kind}-${Date.now()}-${safeName}`;
